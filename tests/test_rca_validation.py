...
from app.services.document_processing.tabular import TabularSelection
...
def test_five_why_rca_checks_completeness_duplicates_and_incident_traceability(monkeypatch):
    ...
    indices = {field['key']: index for index, field in enumerate(rca_validation.RCA_FIELDS)}
    monkeypatch.setattr(rca_validation, 'select_tabular_table', lambda *_: TabularSelection('RCA', 0, rows, indices))
