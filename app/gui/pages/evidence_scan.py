"""Project-scoped Evidence Scan with an internal, version-pinned assessment run."""
from nicegui import app, run, ui
from pathlib import Path
from app.core.screen_registry import screen_url
from sqlalchemy import select

from app.core.config import get_settings
from app.db.database import SessionLocal
from app.db.models import (AuditProject, AuditSession, AuditSessionPracticeArea, ChecklistVersion,
                           Customer, EvidenceFile, EvidenceSource, Finding, PracticeArea, ScanJob)
from app.gui.layout import layout
from app.gui.components.folder_upload import FolderUpload
from app.gui.pages.audit_workspace import _current_user_id, _screen_allowed
from app.services.audit_engine.evidence_scan import scan_session
from app.services.evidence_ingestion import (
    MAX_FOLDER_UPLOAD_BYTES,
    MAX_FOLDER_UPLOAD_FILES,
    persist_uploaded_folder_evidence,
    replace_unreadable_evidence,
)
from app.services.finding_scope import coverage_gaps_query, evidence_findings_query
from app.services.reports.finding_export import create_current_findings_export, display_rule_id

EVIDENCE_SCAN_ACCEPT = '.doc,.docx,.xlsx,.xls,.csv,.txt,.md,.json,.xml,.pptx,.pdf,.jpg,.jpeg,.png,.bmp,.tif,.tiff,.zip'
EVIDENCE_REPLACEMENT_ACCEPT = EVIDENCE_SCAN_ACCEPT.removesuffix(',.zip')
AUTO_SESSION_NAME = 'Evidence Scan (automatic)'


def _update_ui_if_client_alive(client, update) -> bool:
    """Run a post-await UI update only while its originating page still exists.

    A scan is deliberately run in an I/O worker.  The user can navigate away
    or refresh while that work is in progress, which deletes the NiceGUI
    client and its elements.  In that case the scan's persisted database
    result remains valid; attempting to display its completion state must not
    turn it into a UI exception.
    """
    if getattr(client, '_deleted', False):
        return False
    try:
        update()
        return True
    except RuntimeError:
        # The client can be deleted between the guard above and an element
        # update.  Suppress only that lifecycle race; other UI errors should
        # remain visible to the application's exception handler.
        if getattr(client, '_deleted', False):
            return False
        raise


def _active_version(db):
    return db.scalar(select(ChecklistVersion).where(ChecklistVersion.is_active.is_(True)).order_by(ChecklistVersion.id.desc()))


def _unreadable_reason(extracted_text: str | None) -> str:
    """Turn controlled extraction status into an action a reviewer can take."""
    message = (extracted_text or '').strip()
    if '.doc conversion is disabled' in message:
        return 'Legacy .doc conversion is disabled. Convert this file to DOCX, then upload it as a corrected replacement.'
    if message == '[Extraction error: ValueError]':
        return 'This file has a DOCX extension but is not a valid Word document. Open it in Word or LibreOffice, save it again as DOCX, then upload a corrected replacement.'
    if 'contains only Office theme data' in message:
        return 'This is an Office theme package renamed as DOCX; it contains no document text to assess. Upload the original status report or a valid DOCX export.'
    if 'invalid .doc file' in message:
        return 'This file is not a valid legacy Word document. Export it again as DOCX, then upload a corrected replacement.'
    if message:
        return f'The scanner could not read this file ({message.strip("[]")}). Upload a corrected replacement.'
    return 'The scanner could not read this file. Upload a corrected replacement.'


def _workspace_context(db, customer_id: int | None, project_id: int | None,
                       audit_session_id: int | None) -> tuple[Customer, AuditProject, AuditSession] | None:
    """Return a verified customer/project/session chain for the selected workspace.

    The browser stores the three selected IDs independently.  Always verify
    their foreign-key relationship before showing, uploading, or scanning
    evidence so a stale or manipulated selection cannot save data against a
    different audit session.
    """
    if not customer_id or not project_id or not audit_session_id:
        return None
    customer = db.get(Customer, int(customer_id))
    project = db.get(AuditProject, int(project_id))
    audit = db.get(AuditSession, int(audit_session_id))
    if not customer or not project or not audit:
        return None
    if project.customer_id != customer.id or audit.project_id != project.id:
        return None
    return customer, project, audit


def _ensure_scan_session(db, project_id: int, user_id: int) -> AuditSession:
    """Compatibility helper for ruleset reassessments; normal UI uses saved sessions."""
    if not db.get(AuditProject, project_id):
        raise ValueError('Selected project no longer exists.')
    version = _active_version(db)
    if not version:
        raise ValueError('No active ruleset is available. Import the master workbook first.')
    session = db.scalar(select(AuditSession).where(
        AuditSession.project_id == project_id,
        AuditSession.checklist_version_id == version.id,
        AuditSession.audit_name == AUTO_SESSION_NAME,
    ).order_by(AuditSession.id.desc()))
    if session:
        return session
    previous = db.scalar(select(AuditSession).where(
        AuditSession.project_id == project_id,
        AuditSession.audit_name == AUTO_SESSION_NAME,
    ).order_by(AuditSession.id.desc()))
    session = AuditSession(project_id=project_id, checklist_version_id=version.id,
                           audit_name=AUTO_SESSION_NAME, created_by_id=user_id)
    db.add(session); db.flush()
    db.add_all([AuditSessionPracticeArea(audit_session_id=session.id, practice_area_id=area.id)
                for area in db.scalars(select(PracticeArea)).all()])
    if previous:
        prior_files = db.scalars(select(EvidenceFile).join(EvidenceSource).where(
            EvidenceSource.audit_session_id == previous.id)).all()
        if prior_files:
            source = EvidenceSource(audit_session_id=session.id, source_type='ruleset_reassessment',
                                    source_uri=f'assessment:{previous.id}', created_by_id=user_id)
            db.add(source); db.flush()
            db.add_all([EvidenceFile(source_id=source.id, relative_path=item.relative_path,
                                     storage_path=item.storage_path, sha256=item.sha256,
                                     mime_type=item.mime_type, size_bytes=item.size_bytes,
                                     extracted_text=item.extracted_text, processing_status='pending')
                        for item in prior_files])
    return session


def register():
    @ui.page(screen_url('evidence_scan'))
    def evidence_scan_page():
        layout('Evidence Scan', 'Select a workspace, upload evidence, then run and review the scoped assessment.')
        if not _screen_allowed('evidence_scan'):
            ui.label('You do not have permission to view evidence scans.').classes('text-negative'); return

        def load_options():
            with SessionLocal() as db:
                projects = {item.id: item.name for item in db.scalars(select(AuditProject).order_by(AuditProject.name)).all()}
                customers = {item.id: item.name for item in db.scalars(select(Customer).order_by(Customer.name)).all()}
                version = _active_version(db)
            return projects, customers, version

        _, customers, active_version = load_options()
        remembered_project = app.storage.user.get('selected_evidence_project_id')
        remembered_session = app.storage.user.get('selected_audit_session_id')
        remembered_customer = app.storage.user.get('selected_evidence_customer_id')
        with SessionLocal() as db:
            remembered_session_record = db.get(AuditSession, remembered_session) if remembered_session else None
            remembered_project_record = db.get(
                AuditProject, remembered_session_record.project_id if remembered_session_record else remembered_project
            )
        initial_customer = (
            remembered_project_record.customer_id if remembered_project_record else
            remembered_customer if remembered_customer in customers else next(iter(customers), None)
        )
        workspace_before_dialog: dict[str, int | None] = {}
        running_scan_session_id: int | None = None
        with ui.tabs().classes('workflow-tabs w-full max-w-6xl mt-4') as evidence_tabs:
            workspace_tab = ui.tab('1. Workspace', icon='tune').classes('workflow-tab workflow-tab-teal')
            upload_tab = ui.tab('2. Upload evidence', icon='upload_file').classes('workflow-tab workflow-tab-violet')
            review_tab = ui.tab('3. Scan and results', icon='manage_search').classes('workflow-tab workflow-tab-amber')
        with ui.tab_panels(evidence_tabs, value=workspace_tab).classes('w-full max-w-6xl'):
            workspace_panel = ui.tab_panel(workspace_tab)
            upload_panel = ui.tab_panel(upload_tab)
            review_panel = ui.tab_panel(review_tab)
        upload_tab.disable()
        review_tab.disable()

        with workspace_panel:
            with ui.card().classes('w-full max-w-6xl mt-4'):
                ui.label('1. Select audit workspace').classes('text-xl font-bold')
                ui.label('Choose the customer, project, and audit session that own these files and results.').classes('text-sm text-slate-600')
                context_summary = ui.label('No audit workspace selected.').classes('font-medium mt-2')
                rules_summary = ui.label(
                    f'Rules applied: active ruleset {active_version.version}'
                    if active_version else 'No active ruleset is available. Ask an administrator to import the master workbook.'
                ).classes('text-sm text-slate-600')
                with ui.row().classes('w-full gap-3 flex-wrap mt-2'):
                    ui.button('Select audit workspace', icon='tune', on_click=lambda: open_workspace_dialog()).props('color=primary')
                    ui.button('Refresh workspace list', icon='refresh', on_click=lambda: refresh_workspace()).props('outline')

        with ui.dialog() as workspace_dialog:
            with ui.card().classes('w-[900px] max-w-full'):
                ui.label('Select audit workspace').classes('text-xl font-bold')
                ui.label('Choose the saved customer, project, and audit session.').classes('text-sm text-slate-600')
                with ui.row().classes('w-full gap-4 flex-wrap mt-2'):
                    customer_select = ui.select(customers, value=initial_customer, label='Customer').classes('w-60')
                    project_select = ui.select({}, label='Project').classes('w-60')
                    audit_select = ui.select({}, label='Audit session').classes('w-80')
                if not customers:
                    customer_select.disable(); project_select.disable(); audit_select.disable()
                    ui.label('No saved details are available yet. Create an audit workspace from the Add Project menu, then return here.').classes('text-slate-600')
                rule_status = ui.label(
                    f'Active ruleset: {active_version.version} ({active_version.source})'
                    if active_version else 'No active ruleset is available. Ask an administrator to import the master workbook.'
                ).classes('text-sm text-slate-600')
                with ui.row().classes('w-full justify-end gap-2 mt-3'):
                    ui.button('Cancel', on_click=lambda: restore_workspace_selection()).props('flat')
                    ui.button('Use selected workspace', icon='check', on_click=lambda: apply_workspace_selection()).props('color=primary')

        def refresh_audit_options(preferred_session: int | None = None) -> None:
            with SessionLocal() as db:
                sessions = db.scalars(select(AuditSession).where(
                    AuditSession.project_id == int(project_select.value)
                ).order_by(AuditSession.created_at.desc())).all() if project_select.value else []
                options = {
                    item.id: f"{item.audit_name or f'Audit session #{item.id}'}"
                    + (f" — {item.audit_date}" if item.audit_date else '')
                    for item in sessions
                }
                saved_session = app.storage.user.get('selected_audit_session_id')
                selected = preferred_session if preferred_session in options else (
                    saved_session if saved_session in options else next(iter(options), None)
                )
                selected_session = db.get(AuditSession, selected) if selected else None
            audit_select.set_options(options)
            audit_select.value = selected; audit_select.update()
            if selected_session:
                with SessionLocal() as db:
                    version = db.get(ChecklistVersion, selected_session.checklist_version_id)
                rule_status.text = (
                    f'Ruleset pinned to selected audit session: {version.version}'
                    if version else f'Ruleset pinned to selected audit session: ID {selected_session.checklist_version_id}'
                )
                rule_status.update()
            else:
                rule_status.text = (
                    f'Active ruleset: {active_version.version} ({active_version.source})'
                    if active_version else 'No active ruleset is available. Ask an administrator to import the master workbook.'
                )
                rule_status.update()

        def refresh_project_options(preferred_project: int | None = None, preferred_session: int | None = None) -> None:
            with SessionLocal() as db:
                projects = db.scalars(select(AuditProject).where(
                    AuditProject.customer_id == int(customer_select.value)
                ).order_by(AuditProject.name)).all() if customer_select.value else []
                options = {item.id: item.name for item in projects}
                saved_project = app.storage.user.get('selected_evidence_project_id')
                selected = preferred_project if preferred_project in options else (
                    saved_project if saved_project in options else next(iter(options), None)
                )
            project_select.set_options(options)
            project_select.value = selected; project_select.update()
            refresh_audit_options(preferred_session)

        def update_workspace_summary() -> None:
            with SessionLocal() as db:
                context = _workspace_context(db, customer_select.value, project_select.value, audit_select.value)
                version = db.get(ChecklistVersion, context[2].checklist_version_id) if context else None
            if context:
                customer, project, audit = context
                session_name = audit.audit_name or f'Audit session #{audit.id}'
                context_summary.text = f'Selected: {customer.name} / {project.name} / {session_name} (session ID: {audit.id})'
                rules_summary.text = f'Rules applied: {version.version if version else f"ruleset ID {audit.checklist_version_id}"} (pinned to audit session ID {audit.id})'
                scan_context.text = f'Scan target: Customer {customer.name} | Project {project.name} | {session_name} (audit session ID: {audit.id})'
                upload_context.text = f'Upload target: {customer.name} / {project.name} / {session_name} (audit session ID: {audit.id})'
            else:
                context_summary.text = 'No complete audit workspace selected. Select saved details before uploading or scanning.'
                rules_summary.text = 'Rules are applied automatically after an audit session is selected.'
                scan_context.text = 'Scan target: select a complete audit workspace first.'
                upload_context.text = 'Upload target: select a complete audit workspace first.'
            context_summary.update(); rules_summary.update(); scan_context.update(); upload_context.update()

        def refresh_workspace() -> None:
            """Reload saved audit workspaces after a user or administrator change."""
            if running_scan_session_id is not None:
                ui.notify(f'A scan is running for audit session ID {running_scan_session_id}. Wait for it to finish before refreshing the workspace selection.', type='warning')
                return
            with SessionLocal() as db:
                refreshed_customers = {item.id: item.name for item in db.scalars(select(Customer).order_by(Customer.name)).all()}
            customer_select.set_options(refreshed_customers)
            if customer_select.value not in refreshed_customers:
                customer_select.value = next(iter(refreshed_customers), None)
            customer_select.update()
            refresh_project_options()
            update_workspace_summary()
            sync_action_controls()
            results.refresh()

        def open_workspace_dialog() -> None:
            if running_scan_session_id is not None:
                ui.notify(f'A scan is running for audit session ID {running_scan_session_id}. Workspace selection is locked until it finishes.', type='warning')
                return
            workspace_before_dialog.clear()
            workspace_before_dialog.update({
                'customer_id': customer_select.value,
                'project_id': project_select.value,
                'audit_session_id': audit_select.value,
            })
            workspace_dialog.open()

        def restore_workspace_selection() -> None:
            """Make Cancel a real cancel even though the dialog uses live selects."""
            customer_id = workspace_before_dialog.get('customer_id')
            project_id = workspace_before_dialog.get('project_id')
            session_id = workspace_before_dialog.get('audit_session_id')
            customer_select.value = customer_id
            customer_select.update()
            refresh_project_options(project_id, session_id)
            for key, value in (
                ('selected_evidence_customer_id', customer_id),
                ('selected_evidence_project_id', project_id),
                ('selected_audit_session_id', session_id),
            ):
                if value is None:
                    app.storage.user.pop(key, None)
                else:
                    app.storage.user[key] = value
            update_workspace_summary()
            sync_action_controls()
            results.refresh()
            workspace_dialog.close()

        def apply_workspace_selection() -> None:
            if running_scan_session_id is not None:
                ui.notify(f'A scan is running for audit session ID {running_scan_session_id}. Workspace selection is locked until it finishes.', type='warning')
                return
            with SessionLocal() as db:
                context = _workspace_context(db, customer_select.value, project_select.value, audit_select.value)
            if not context:
                ui.notify('Choose a valid customer, project, and audit session combination.', type='warning')
                refresh_workspace()
                return
            customer, project, audit = context
            app.storage.user['selected_evidence_customer_id'] = customer.id
            app.storage.user['selected_evidence_project_id'] = project.id
            app.storage.user['selected_audit_session_id'] = audit.id
            update_workspace_summary()
            sync_action_controls()
            results.refresh()
            workspace_dialog.close()

        def download_current_findings(fmt: str) -> None:
            if not audit_select.value:
                ui.notify('Select an audit workspace first.', type='warning')
                return
            try:
                with SessionLocal() as db:
                    context = _workspace_context(db, customer_select.value, project_select.value, audit_select.value)
                    if not context:
                        raise ValueError('The selected customer, project, and audit session no longer form a valid workspace.')
                    report = create_current_findings_export(db, context[2].id, _current_user_id(), fmt)
                media_type = 'text/csv' if fmt == 'csv' else 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
                ui.download(report.storage_path, filename=Path(report.storage_path).name, media_type=media_type)
                ui.notify(f'{fmt.upper()} download is ready.', type='positive')
            except Exception as exc:
                ui.notify(f'Could not create findings download: {exc}', type='negative')

        @ui.refreshable
        def results() -> None:
            if not audit_select.value:
                ui.label('Select a saved customer, project, and audit session before uploading or reviewing stored evidence.').classes('text-slate-600 mt-5'); return
            with SessionLocal() as db:
                context = _workspace_context(db, customer_select.value, project_select.value, audit_select.value)
                if not context:
                    ui.label('The selected workspace is no longer valid. Refresh the saved details and choose it again.').classes('text-negative mt-5'); return
                customer, project, session = context
                files = db.scalars(select(EvidenceFile).join(EvidenceSource).where(
                    EvidenceSource.audit_session_id == session.id,
                ).order_by(EvidenceFile.relative_path, EvidenceFile.id)).all()
                findings = db.scalars(evidence_findings_query(session.id).order_by(Finding.created_at.desc())).all()
                coverage_gaps = db.scalars(coverage_gaps_query(session.id).order_by(Finding.created_at.desc())).all()
                scan_jobs = db.scalars(select(ScanJob).where(
                    ScanJob.audit_session_id == session.id,
                    ScanJob.job_type == 'master_rule_and_irp_scan',
                ).order_by(ScanJob.created_at.desc(), ScanJob.id.desc())).all()
                latest_job = scan_jobs[0] if scan_jobs else None
                version = db.get(ChecklistVersion, session.checklist_version_id)
            session_name = session.audit_name or f'Audit session #{session.id}'
            ui.label(f'Saved results — {session_name} (session ID: {session.id})').classes('text-xl font-bold mt-2')
            ui.label(
                f'{customer.name} / {project.name} · pinned ruleset '
                f'{version.version if version else session.checklist_version_id}'
            ).classes('text-sm text-slate-600')
            with ui.row().classes('w-full gap-4 flex-wrap mt-2'):
                ui.label(f'{len(files)} stored document(s)').classes('text-lg font-bold')
                ui.label(f"{sum(item.processing_status == 'processed' for item in files)} assessed").classes('text-slate-700')
                ui.label(f"{sum(item.processing_status == 'pending' for item in files)} waiting for scan").classes('text-slate-700')
                if any(item.processing_status == 'error' for item in files):
                    ui.label(f"{sum(item.processing_status == 'error' for item in files)} needs attention").classes('text-negative')
            unreadable_by_hash: dict[str, EvidenceFile] = {}
            for item in files:
                if item.processing_status == 'error':
                    current = unreadable_by_hash.get(item.sha256)
                    if current is None or item.id < current.id:
                        unreadable_by_hash[item.sha256] = item
            unreadable_files = list(unreadable_by_hash.values())
            if unreadable_files:
                ui.label('Some files could not be read. Replace them before relying on coverage gaps.').classes('text-sm text-amber-800 mt-2')
                with ui.expansion(f'Unreadable evidence — {len(unreadable_files)} unique file(s)', icon='warning_amber').classes('w-full mt-1'):
                    for item in sorted(unreadable_files, key=lambda evidence: evidence.relative_path.casefold()):
                        ui.label(f'{item.relative_path} — {_unreadable_reason(item.extracted_text)}').classes('block text-sm text-amber-900 p-2')
            if not files:
                ui.label('No evidence has been stored for this audit session yet. Use one of the upload choices above.').classes('text-slate-600 mt-2')
                return
            role_names = {
                'PROJECT_IMPLEMENTATION_EVIDENCE': 'Project evidence',
                'PROCESS_REFERENCE': 'Process reference',
                'TEMPLATE': 'Template',
                'BLANK_TEMPLATE': 'Blank template',
                'DUPLICATE': 'Duplicate',
            }
            status_names = {
                'pending': 'Stored — waiting for scan',
                'processed': 'Assessed',
                'error': 'Could not assess',
                'duplicate': 'Duplicate content — skipped',
                'replaced': 'Replaced by corrected evidence',
            }
            rows=[]
            for file in files:
                classification = file.classification_json or {}
                assessed = file.processing_status == 'processed' and bool(classification)
                rows.append({
                    'id': file.id,
                    'file': file.relative_path,
                    'type': classification.get('detected_type', 'Not assessed — run scan') if assessed else 'Not assessed — run scan',
                    'role': role_names.get(classification.get('document_role'), 'Not assessed') if assessed else 'Not assessed',
                    'practice_areas': ', '.join(classification.get('practice_areas', [])) or ('Unclassified' if assessed else 'Not assessed'),
                    'confidence': f"{classification.get('confidence')} ({classification.get('confidence_score')}%)" if assessed else 'Not assessed',
                    'status': status_names.get(file.processing_status, file.processing_status.title()),
                })
            ui.table(columns=[{'name':key,'label':label,'field':key,'align':'left'} for key,label in (
                ('file','Document'),('type','Document Type'),('role','Document Role'),('practice_areas','Practice Areas'),
                ('confidence','Confidence'),('status','Processing Status'))],rows=rows,row_key='id').props("pagination={'rowsPerPage': 25, 'rowsPerPageOptions': [10, 25, 50, 100]}").classes('w-full')
            if unreadable_files:
                with ui.card().classes('w-full mt-3 border border-amber-200'):
                    ui.label('Replace unreadable evidence').classes('font-bold text-amber-900')
                    ui.label('Select the unreadable file and upload its corrected replacement.').classes('text-sm text-slate-600')
                    replacement_select = ui.select(
                        {item.id: item.relative_path for item in unreadable_files},
                        label='Unreadable file to replace',
                    ).classes('w-full max-w-2xl')

                    def upload_replacement(event) -> None:
                        if not _screen_allowed('evidence_scan', 'write'):
                            ui.notify('You do not have permission to replace evidence.', type='negative')
                            return
                        if running_scan_session_id is not None:
                            ui.notify('Wait for the running scan to finish before replacing evidence.', type='warning')
                            return
                        if not replacement_select.value:
                            ui.notify('Select the unreadable file you are replacing first.', type='warning')
                            return
                        try:
                            with SessionLocal() as db:
                                context = _workspace_context(db, customer_select.value, project_select.value, audit_select.value)
                                if not context:
                                    raise ValueError('The selected audit workspace is no longer valid.')
                                _, _, selected_session = context
                                selected_file = db.scalar(select(EvidenceFile).join(EvidenceSource).where(
                                    EvidenceFile.id == int(replacement_select.value),
                                    EvidenceSource.audit_session_id == selected_session.id,
                                ))
                                if not selected_file:
                                    raise ValueError('The selected unreadable file does not belong to this audit session.')
                                outcome = replace_unreadable_evidence(
                                    db, selected_file.id, _current_user_id(), event.name,
                                    event.content.read(), event.type or None,
                                )
                                db.commit()
                            replacement_uploader.reset()
                            results.refresh()
                            ui.notify(f'Added corrected evidence file #{outcome.evidence_file_ids[0]}. Run the scan again to assess it.', type='positive')
                        except Exception as exc:
                            ui.notify(f'Could not replace unreadable evidence: {exc}', type='negative')

                    replacement_uploader = ui.upload(
                        on_upload=upload_replacement, auto_upload=True,
                        max_file_size=get_settings().max_upload_mb * 1024 * 1024,
                        label='Upload corrected replacement file',
                    ).props(f'accept={EVIDENCE_REPLACEMENT_ACCEPT}').classes('w-full max-w-xl')
                    if not _screen_allowed('evidence_scan', 'write'):
                        replacement_uploader.disable()
            with ui.expansion('What do these labels mean?', icon='help_outline').classes('w-full mt-2'):
                ui.label('Document type is the best content match against this session\'s approved 14-document catalogue. Its mapped practice area is the only CMMI area assessed for that file. Confidence measures the document match only; it is not an audit score or compliance result.').classes('text-sm text-slate-600')
                ui.label('Document role tells the scanner whether a file is usable project evidence, a process reference, a template, blank, or a duplicate. Templates and references are retained but do not satisfy implementation evidence requirements.').classes('text-sm text-slate-600')
                ui.label('Practice areas are CMMI areas suggested by the stored document-type rule. “Unclassified” means the scanner could not make a reliable mapping and needs a reviewer. Processing status shows whether the stored file has been assessed successfully.').classes('text-sm text-slate-600')
            if not latest_job:
                ui.label('No scan result yet. The stored files have not been compared with the approved document catalogue and its mapped CMMI rules.').classes('text-slate-600 mt-3')
            elif latest_job.status == 'completed':
                summary = latest_job.result_summary or {}
                ui.label(
                    f"Latest scan job #{latest_job.id} completed: {summary.get('files_processed', 0)} file(s) assessed, "
                    f"{summary.get('evidence_findings_created', summary.get('findings_created', 0))} file-backed finding(s), and "
                    f"{summary.get('coverage_gaps_created', 0)} checklist coverage gap(s)."
                ).classes('font-medium text-positive mt-3')
                if summary.get('duplicate_files_skipped', 0):
                    ui.label(f"{summary['duplicate_files_skipped']} pre-existing duplicate evidence file(s) were retained in history and skipped from this scan.").classes('text-sm text-slate-600')
            elif latest_job.status == 'running':
                ui.label('A scan is currently running. The most recent successful file-backed findings and checklist coverage gaps remain visible below for this session.').classes('text-primary mt-3')
            else:
                ui.label(f"The latest scan did not complete: {(latest_job.result_summary or {}).get('error', 'unknown error')}. The most recent successful file-backed findings and checklist coverage gaps remain visible below for this session.").classes('text-negative mt-3')
            ui.separator().classes('my-4')
            ui.label(f'Evidence findings from uploaded files — audit session ID {session.id}').classes('text-lg font-bold')
            ui.label('The Excel download keeps the AFR summary and adds detailed control findings plus evidence classification on separate sheets. CSV remains summary-only.').classes('text-sm text-slate-600')
            if not findings:
                ui.label('No current file-backed findings are recorded for this audit session.').classes('text-positive mt-2')
            else:
                with ui.row().classes('w-full items-center gap-3 mt-2'):
                    ui.label(f'{len(findings)} file-backed finding(s)').classes('font-medium')
                    ui.button('Download CSV', icon='download', on_click=lambda: download_current_findings('csv')).props('outline')
                    ui.button('Download Excel', icon='download', on_click=lambda: download_current_findings('xlsx')).props('outline color=primary')
                finding_rows = [{
                    'id': finding.id,
                    'rule': finding.rule_id or 'Specialized validation',
                    'practice_area': finding.practice_area_code,
                    'severity': finding.severity.title(),
                    'finding': finding.title,
                    'recommendation': finding.recommendation,
                    'status': finding.status.title(),
                } for finding in findings]
                ui.table(columns=[{'name': key, 'label': label, 'field': key, 'align': 'left'} for key, label in (
                    ('rule', 'Rule'), ('practice_area', 'Practice Area'), ('severity', 'Severity'),
                    ('finding', 'Finding'), ('recommendation', 'Recommended action'), ('status', 'Status'),
                )], rows=finding_rows, row_key='id').props("pagination={'rowsPerPage': 25, 'rowsPerPageOptions': [10, 25, 50, 100]}").classes('w-full')
            with ui.expansion(f'Document and checklist coverage — {len(coverage_gaps)}', icon='rule').classes('w-full mt-4'):
                ui.label('These rows identify governed evidence that was not uploaded. They are included in the AFR Open Findings so the required document is visible to the auditor.').classes('text-sm text-slate-600 p-2')
                if coverage_gaps:
                    gap_rows = [{
                        'id': finding.id, 'rule': display_rule_id(finding.rule_id) or 'Specialized validation',
                        'practice_area': finding.practice_area_code, 'severity': finding.severity.title(),
                        'status': finding.status.title(), 'finding': finding.title,
                    } for finding in coverage_gaps]
                    ui.table(columns=[{'name': key, 'label': label, 'field': key, 'align': 'left'} for key, label in (
                        ('rule', 'Rule'), ('practice_area', 'Practice Area'), ('severity', 'Severity'),
                        ('status', 'Status'), ('finding', 'Coverage gap'),
                    )], rows=gap_rows, row_key='id').props("pagination={'rowsPerPage': 25, 'rowsPerPageOptions': [10, 25, 50, 100]}").classes('w-full')
                else:
                    ui.label('No open checklist coverage gaps are recorded for this audit session.').classes('text-positive p-2')
            if scan_jobs:
                with ui.expansion('Scan history', icon='history').classes('w-full mt-4'):
                    ui.label('Every scan remains recorded for this audit session.').classes('text-sm text-slate-600')
                    history_rows = []
                    for job in scan_jobs:
                        summary = job.result_summary or {}
                        history_rows.append({
                            'id': job.id,
                            'job_id': job.id,
                            'started': job.created_at.strftime('%Y-%m-%d %H:%M UTC'),
                            'status': job.status.title(),
                            'files': summary.get('files_processed', summary.get('files_total', '—')),
                            'findings': summary.get('evidence_findings_created', summary.get('findings_created', '—')),
                            'coverage_gaps': summary.get('coverage_gaps_created', '—'),
                            'detail': summary.get('error') or summary.get('phase') or 'Completed assessment',
                        })
                    ui.table(columns=[{'name': key, 'label': label, 'field': key, 'align': 'left'} for key, label in (
                        ('job_id', 'Scan job ID'), ('started', 'Started'), ('status', 'Status'), ('files', 'Files'), ('findings', 'File-backed findings'), ('coverage_gaps', 'Coverage gaps'), ('detail', 'Detail'),
                    )], rows=history_rows, row_key='id').props("pagination={'rowsPerPage': 10, 'rowsPerPageOptions': [10, 25, 50, 100]}").classes('w-full mt-2')

        with upload_panel:
            with ui.card().classes('w-full max-w-6xl mt-4'):
                ui.label('2. Upload evidence').classes('text-xl font-bold')
                ui.label('Add individual files, multiple files, a project ZIP, or an uncompressed project folder to the selected workspace.').classes('text-sm text-slate-600')
                upload_context = ui.label('Upload target: select a complete audit workspace first.').classes('text-sm font-medium mt-2')
                def upload_project_evidence(event) -> None:
                    if not _screen_allowed('evidence_scan', 'write'):
                        ui.notify('You do not have permission to upload evidence.', type='negative'); return
                    if running_scan_session_id is not None:
                        ui.notify(f'A scan is running for audit session ID {running_scan_session_id}. Upload evidence after it finishes so the next scan includes the new files.', type='warning'); return
                    if not audit_select.value:
                        ui.notify('Select a customer, project, and audit session first.', type='warning'); return
                    try:
                        selected_files = []
                        for relative_path, content, mime_type in zip(event.names, event.contents, event.types):
                            normalized_path = relative_path.replace('\\', '/')
                            selected_files.append((normalized_path, normalized_path.rsplit('/', maxsplit=1)[-1],
                                                   content.read(), mime_type or None))
                        with SessionLocal() as db:
                            context = _workspace_context(db, customer_select.value, project_select.value, audit_select.value)
                            if not context:
                                raise ValueError('The selected customer, project, and audit session no longer form a valid workspace.')
                            _, _, session = context
                            outcome = persist_uploaded_folder_evidence(
                                db, session.id, _current_user_id(), selected_files,
                            )
                            db.commit()
                        results.refresh()
                        message = f'Added {outcome.files_persisted} evidence file(s) to the selected audit session.'
                        if outcome.duplicates_skipped:
                            message += f' Skipped {outcome.duplicates_skipped} exact duplicate(s).'
                        message += ' Stored files are waiting for scan.'
                        ui.notify(message, type='positive')
                    except Exception as exc:
                        ui.notify(f'Evidence upload failed: {exc}',type='negative')
                uploader = FolderUpload(
                    on_multi_upload=upload_project_evidence,
                    accept=EVIDENCE_SCAN_ACCEPT,
                    max_file_size=get_settings().max_upload_mb * 1024 * 1024,
                    max_total_size=MAX_FOLDER_UPLOAD_BYTES,
                    max_files=MAX_FOLDER_UPLOAD_FILES,
                    label='Upload evidence',
                ).classes('w-full max-w-lg mt-3')
                if not audit_select.value or not _screen_allowed('evidence_scan', 'write'):
                    uploader.disable()

        @ui.refreshable
        def scan_progress() -> None:
            progress_session_id = running_scan_session_id or audit_select.value
            if not progress_session_id:
                return
            with SessionLocal() as db:
                job = db.scalars(select(ScanJob).where(
                    ScanJob.audit_session_id == int(progress_session_id),
                    ScanJob.job_type == 'master_rule_and_irp_scan',
                ).order_by(ScanJob.created_at.desc(), ScanJob.id.desc())).first()
            if not job or job.status != 'running':
                return
            progress = job.result_summary or {}
            completed = int(progress.get('files_processed') or 0)
            total = int(progress.get('files_total') or 0)
            ui.label(f"Audit session ID {progress_session_id} — processing document {completed} of {total}: {progress.get('current_file') or 'preparing scan'}").classes('text-primary text-sm mt-2')
            if total:
                ui.linear_progress(value=completed / total).classes('w-full')

        async def run_scan() -> None:
            nonlocal running_scan_session_id
            page_client = ui.context.client
            if not _screen_allowed('evidence_scan', 'write'):
                ui.notify('You do not have permission to run an evidence scan.',type='negative'); return
            if running_scan_session_id is not None:
                ui.notify(f'A scan is already running for audit session ID {running_scan_session_id}.', type='warning'); return
            if not audit_select.value:
                ui.notify('Select a customer, project, and audit session first.',type='warning'); return
            audit_session_id = int(audit_select.value)
            customer_id = int(customer_select.value) if customer_select.value else None
            project_id = int(project_select.value) if project_select.value else None
            with SessionLocal() as db:
                if not _workspace_context(db, customer_id, project_id, audit_session_id):
                    ui.notify('The selected customer, project, and audit session no longer form a valid workspace.', type='negative')
                    return
            running_scan_session_id = audit_session_id
            scan_button.disable()
            uploader.disable()
            scan_spinner.set_visibility(True)
            scan_state.text = f'Scan is running for audit session ID {audit_session_id}. Each stored file is matched to the approved catalogue, then checked only against its mapped CMMI practice area.'
            scan_state.classes(remove='text-negative', add='text-primary')
            scan_state.update()
            scan_progress.refresh()
            try:
                user_id = _current_user_id()
                def execute_scan() -> dict:
                    with SessionLocal() as db:
                        if not _workspace_context(db, customer_id, project_id, audit_session_id):
                            raise ValueError('The selected customer, project, and audit session no longer form a valid workspace.')
                        return scan_session(db, audit_session_id, user_id)
                outcome = await run.io_bound(execute_scan)
            except Exception as exc:
                def show_failure() -> None:
                    scan_state.text = f'Evidence scan did not complete for audit session ID {audit_session_id}: {exc}'
                    scan_state.classes(remove='text-primary text-positive', add='text-negative')
                    scan_state.update()
                    scan_progress.refresh()
                    ui.notify(f'Evidence scan failed: {exc}', type='negative')
                _update_ui_if_client_alive(page_client, show_failure)
            else:
                direct_count = outcome.get('evidence_findings_created', outcome['findings_created'])
                coverage_count = outcome.get('coverage_gaps_created', 0)
                skipped_duplicates = outcome.get('duplicate_files_skipped', 0)
                def show_completion() -> None:
                    scan_state.text = f"Scan completed for audit session ID {audit_session_id}: {outcome['files_processed']} file(s) assessed, {direct_count} file-backed finding(s), and {coverage_count} checklist coverage gap(s)." + (f" {skipped_duplicates} duplicate stored file(s) were skipped." if skipped_duplicates else '') + ' Review the persisted result below.'
                    scan_state.classes(remove='text-primary text-negative', add='text-positive')
                    scan_state.update()
                    scan_progress.refresh()
                    results.refresh()
                    ui.notify(f"Evidence scan completed for session ID {audit_session_id}: {direct_count} file-backed finding(s), {coverage_count} coverage gap(s)" + (f", {skipped_duplicates} duplicate(s) skipped." if skipped_duplicates else '.'),type='positive')
                _update_ui_if_client_alive(page_client, show_completion)
            finally:
                running_scan_session_id = None
                _update_ui_if_client_alive(
                    page_client,
                    lambda: (scan_spinner.set_visibility(False), sync_action_controls()),
                )
        with review_panel:
            with ui.card().classes('w-full max-w-6xl mt-4'):
                ui.label('3. Run scan and review results').classes('text-xl font-bold')
                ui.label('The scan evaluates each stored file only against its mapped approved document type and practice area.').classes('text-sm text-slate-600')
                scan_context = ui.label('Scan target: select a complete audit workspace first.').classes('text-sm font-medium text-slate-700 mt-2')
                with ui.row().classes('items-center gap-3 mt-2'):
                    scan_button=ui.button('Run Scoped CMMI Evidence Scan',icon='manage_search',on_click=run_scan).props('color=primary')
                    scan_spinner = ui.spinner('dots', size='lg')
                    scan_spinner.set_visibility(False)
                    scan_state = ui.label('Ready to scan stored evidence.').classes('text-sm text-slate-600')
                scan_progress()
                if not audit_select.value or not _screen_allowed('evidence_scan', 'write'): scan_button.disable()

        def sync_action_controls() -> None:
            if audit_select.value:
                upload_tab.enable()
                review_tab.enable()
            else:
                upload_tab.disable()
                review_tab.disable()
            if audit_select.value and _screen_allowed('evidence_scan', 'write') and running_scan_session_id is None:
                uploader.enable()
            else:
                uploader.disable()
            if audit_select.value and _screen_allowed('evidence_scan', 'write') and running_scan_session_id is None:
                scan_button.enable()
            else:
                scan_button.disable()

        def select_customer() -> None:
            if customer_select.value:
                app.storage.user['selected_evidence_customer_id'] = customer_select.value
            refresh_project_options()
            update_workspace_summary()
            sync_action_controls()
            results.refresh()

        def select_project() -> None:
            if project_select.value:
                app.storage.user['selected_evidence_project_id'] = project_select.value
            else:
                app.storage.user.pop('selected_evidence_project_id', None)
            refresh_audit_options()
            update_workspace_summary()
            sync_action_controls()
            results.refresh()

        def select_audit() -> None:
            if audit_select.value:
                app.storage.user['selected_audit_session_id'] = audit_select.value
            else:
                app.storage.user.pop('selected_audit_session_id', None)
            update_workspace_summary()
            sync_action_controls()
            results.refresh()

        customer_select.on('update:model-value', select_customer)
        project_select.on('update:model-value', select_project)
        audit_select.on('update:model-value', select_audit)
        refresh_project_options()
        sync_action_controls()
        update_workspace_summary()
        with review_panel:
            with ui.card().classes('w-full max-w-6xl mt-4'):
                results()
        ui.timer(1.0, scan_progress.refresh)
