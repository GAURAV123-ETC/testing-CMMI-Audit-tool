...
Copy `.env.example` to `.env` and set `SECRET_KEY` and MySQL credentials. Local development defaults to port `8090`; set `APP_HOST`, `APP_PORT`, `PUBLIC_BASE_URL`, and `MICROSOFT_REDIRECT_URI` to the externally reachable values in each environment. The normal database configuration is the `DB_*` variable set; `DATABASE_URL` is an optional test/deployment override. On the first empty-database visit to `/login`, create the initial administrator account; there are no bootstrap passwords stored in environment configuration.
